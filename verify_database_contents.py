        ...
        with conn.cursor() as cur:
            ...
            for table in person_tables:
                ...
                for row in rows[:5]:  # Show first 5 records
                    print(f"     - Handle: {row[0]}, ID: {row[1]}, Name: {row[2]} {row[3]}")
                if len(rows) > 5:
            ...
            for table in person_tables:
                ...
                result = cur.fetchone()
                print(f"   {tree_name}: {result[0]} total records, {result[1]} unique surnames")
                ...
                contamination = [s for s in surnames if s[0] and s[0] != expected_surname and s[0] != 'Concurrent' and not s[0].startswith('Test')]
                
                if contamination:
                    print(f"     ⚠️  WARNING: Found unexpected surnames: {contamination}")
                else:
                    print(f"     ✓ No cross-contamination detected")
            
            # 4. Check other object types
            print("\n4. OTHER OBJECT TYPES:")
            object_types = ['family', 'event', 'place', 'source', 'citation', 'repository', 'media', 'note', 'tag']
            ...
            for obj_type in object_types:
                ...
                if obj_tables:
                    ...
                    if total_count > 0:
                        print(f"   {obj_type.title()}: {total_count} total records across {len(obj_tables)} tables")
            ...
            try:
                ...
                with db_conn.cursor() as cur:
                    ...
                    if 'person' in tables:
                        ...
                        """)
                        result = cur.fetchone()
                        print(f"  Person records: {result[0]} total, {result[1]} unique IDs")
                    
                    if 'metadata' in tables:
                        cur.execute("SELECT setting, value FROM metadata WHERE setting = 'database_mode'")
                        mode = cur.fetchone()
                        if mode:
