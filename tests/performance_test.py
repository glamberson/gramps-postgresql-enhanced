...
"""
    ...
    def test_bulk_insert(self, count=1000):
        ...
        self.results["bulk_insert"].append(
            {"count": count, "duration": duration, "rate": rate}
        )
    ...
    def test_individual_inserts(self, count=100):
        ...
        print(
            f"Inserted {count} persons individually in {duration:.2f}s ({rate:.1f} persons/sec)"
        )
    ...
    def test_search_performance(self, search_count=100):
        ...
        print(
            f"Handle lookups: {search_count} in {duration:.2f}s ({rate:.1f} lookups/sec)"
        )

        self.results["handle_lookup"].append(
            {"count": search_count, "duration": duration, "rate": rate}
        )
        ...
        self.results["surname_search"].append(
            {"surnames": len(surnames), "duration": duration}
        )
    ...
    def test_update_performance(self, update_count=100):
        ...
        print(
            f"Updated {update_count} persons in {duration:.2f}s ({rate:.1f} updates/sec)"
        )

        self.results["update"].append(
            {"count": update_count, "duration": duration, "rate": rate}
        )
    ...
    def test_family_performance(self, family_count=100):
        ...
        print(
            f"Created {family_count} families in {duration:.2f}s ({rate:.1f} families/sec)"
        )

        self.results["family_create"].append(
            {"count": family_count, "duration": duration, "rate": rate}
        )
    ...
    def test_complex_queries(self):
        ...
        print(
            f"Gender count: {male_count} males, {female_count} females in {duration:.2f}s"
        )
        ...
        largest_surname = max(
            surname_groups.keys(), key=lambda s: len(surname_groups[s])
        )

        print(f"Surname grouping: {len(surname_groups)} surnames in {duration:.2f}s")
        print(
            f"Largest group: {largest_surname} with {len(surname_groups[largest_surname])} persons"
        )
    ...
    def test_concurrent_access(self, concurrent_readers=5):
        ...
        def reader_thread(thread_id, count=100):
            ...
            for _ in range(count):
                ...
                with conn.cursor() as cur:
                    cur.execute(
                        "SELECT json_data FROM person WHERE handle = %s", [handle]
                    )
                    result = cur.fetchone()
            ...
            results_queue.put(
                {"thread_id": thread_id, "duration": thread_duration, "reads": count}
            )
    ...
    def generate_report(self):
        ...
        if "individual_insert" in self.results:
            ...
            print(
                f"Individual Insert Rate: {statistics.mean(ind_rates):.1f} persons/sec"
            )

        if "handle_lookup" in self.results:
            lookup_rates = [r["rate"] for r in self.results["handle_lookup"]]
            print(
                f"Handle Lookup Rate: {statistics.mean(lookup_rates):.1f} lookups/sec"
            )
        ...
        if "family_create" in self.results:
            ...
            print(
                f"Family Creation Rate: {statistics.mean(family_rates):.1f} families/sec"
            )
