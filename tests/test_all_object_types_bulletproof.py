#!/usr/bin/env python3
"""
BULLETPROOF TEST FOR ALL GRAMPS OBJECT TYPES
Tests every Gramps object type with comprehensive data to ensure 100% data integrity.

This test verifies:
1. All 9 Gramps object types (Person, Family, Event, Place, Source, Citation, Repository, Media, Note, Tag)
2. Complex relationships and cross-references
3. Unicode, special characters, and edge cases
4. Data integrity through store/retrieve cycles
"""
...
def create_comprehensive_person():
    """Create a Person with ALL possible fields populated."""
    person = Person()
    ...
    birth_date = Date()
    birth_date.set_yr_mon_day(1950, 6, 15)
    person.set_birth_ref_index(0)
    
    death_date = Date()
    death_date.set_yr_mon_day(2020, 12, 31)
    person.set_death_ref_index(0)
...
def create_comprehensive_family():
    ...
    father_handle = generate_handle()
    mother_handle = generate_handle()
...
def create_comprehensive_event():
    ...
    event.set_place_handle(place_ref.ref)
...
def create_comprehensive_place():
    """Create a Place with hierarchies and coordinates."""
    place = Place()
    place.set_handle(generate_handle())
    place.set_gramps_id(f"P{random.randint(1000, 9999)}")
    
    # Place names in multiple languages
    place.set_name("München / Munich / 慕尼黑")
    place.set_title("Capital of Bavaria")
    place.set_code("MUC")
    
    # GPS coordinates
    place.set_latitude("48.1351°N")
    place.set_longitude("11.5820°E")
    
    # Place hierarchy (enclosed by)
    parent_place = generate_handle()
    place.add_placeref(PlaceRef(parent_place))
...
def create_comprehensive_source():
    """Create a Source with all fields."""
    source = Source()
    ...
    repo_ref.set_media_type(SourceMediaType.MICROFILM)
...
def create_comprehensive_citation():
    ...
    # Notes with analysis
    note_list = [generate_handle()]
    citation.set_note_list(note_list)
...
def create_comprehensive_note():
    """Create a Note with formatted text and special characters."""
    note = Note()
    ...
    # Complex formatted text with unicode, HTML-like content, and special characters
...
• Null byte: \x00 (should be handled)
...
End of comprehensive test note."""
    ...
    # Create StyledText with tags
    styled_text = StyledText(text)
    
    # Add some styled tags
    tag1 = StyledTextTag()
    tag1.set_name(StyledTextTagType.BOLD)
    tag1.set_ranges([(0, 20)])  # Make title bold
    styled_text.add_tag(tag1)
    
    tag2 = StyledTextTag()
    tag2.set_name(StyledTextTagType.ITALIC)
    tag2.set_ranges([(50, 70)])  # Make some text italic
...
def test_all_object_types():
    ...
    print("=" * 80)
    print("BULLETPROOF TEST FOR ALL GRAMPS OBJECT TYPES")
    ...
    config = {
        ...
        "database": f"bulletproof_all_types_{datetime.now().strftime('%Y%m%d_%H%M%S')}",
    ...
    import os
    test_dir = f"/tmp/test_all_types_{datetime.now().strftime('%Y%m%d_%H%M%S')}"
