    ...
    def setup(self):
        ...
        try:
            ...
            with admin_conn.cursor() as cur:
                ...
                # Create fresh
                cur.execute(
                    sql.SQL("CREATE DATABASE {}").format(sql.Identifier(self.shared_db))
                )
...
"""
    ...
    def test_data_isolation(self):
        ...
        try:
            ...
            for i, tree_name in enumerate(TEST_TREES):
                ...
                print(
                    f"  ✓ Added {person.get_primary_name().get_first_name()} to {tree_name}"
                )
            ...
            for tree_name in TEST_TREES:
                ...
                print(
                    f"  ✓ {tree_name}: Found only its own person ({person.get_primary_name().get_first_name()})"
                )
    ...
    def test_concurrent_access(self):
        ...
        try:
            ...
            for tree_name in TEST_TREES:
                thread = threading.Thread(target=add_people_to_tree, args=(tree_name,))
                thread.start()
            ...
            for tree_name in TEST_TREES:
                ...
                if count != expected:
                    raise Exception(
                        f"{tree_name} has {count} people, expected {expected}"
                    )
    ...
    def test_performance_comparison(self):
        ...
        try:
            ...
            for i in range(100):
                person = create_test_person(
                    f"PERF_{i:04d}", f"P{i:04d}", f"Perf{i}", "Test"
                )
