...
class QueryProfiler:
    ...
    def __init__(self):
        ...
        self.slow_query_threshold = float(os.environ.get('GRAMPS_POSTGRESQL_SLOW_QUERY', '0.1'))
...
class ConnectionMonitor:
    ...
    def get_connection_health(self):
        ...
        return {
            ...
            'last_event': self.connection_events[-1] if self.connection_events else None
        }
...
class DebugContext:
    ...
    def log_query(self, sql, params=None, duration=None, rows_affected=None):
        ...
        if duration is None:
            ...
            query_info = self.query_profiler.record_query(sql, params, duration, rows_affected)
            self.logger.debug(
...
def timed_method(method):
    ...
    def wrapper(self, *args, **kwargs):
        if hasattr(self, '_debug_context') and self._debug_context:
            with self._debug_context.operation(f"{self.__class__.__name__}.{method.__name__}"):
