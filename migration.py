...
class MigrationManager:
    ...
    def migrate_from_sqlite(self, sqlite_path, callback=None):
        ...
        try:
            ...
            for obj_type in OBJECT_TYPES:
                cursor = source.execute(f"SELECT COUNT(*) FROM {obj_type}")
            ...
            try:
                ...
                for obj_type in OBJECT_TYPES:
                    if callback:
                        callback(current, total_objects, 
                                _(f"Migrating {obj_type} objects..."))
                    ...
                    if callback:
                        callback(current, total_objects, 
                                _(f"Migrated {count} {obj_type} objects"))
    ...
    def _object_to_json(self, obj, obj_type):
        ...
        json_data = {
            'handle': data[0] if isinstance(data, (list, tuple)) else data.get('handle'),
            'gramps_id': data[1] if isinstance(data, (list, tuple)) and len(data) > 1 else data.get('gramps_id'),
            '_class': obj_type,
        }
    ...
    def _migrate_references(self, source):
        ...
        for row in cursor:
            ...
            """, [row['obj_handle'], row['obj_class'], 
                  row['ref_handle'], row['ref_class']])
    ...
    def _migrate_additional_tables(self, source):
        ...
        if self._table_exists_in_sqlite(source, 'gender_stats'):
            ...
            for row in cursor:
                ...
                """, [row['given_name'], row['male'], 
                      row['female'], row['unknown']])
    ...
    def upgrade_to_enhanced(self, callback=None):
        ...
        for obj_type in OBJECT_TYPES:
            if callback:
                callback(current, total_objects, 
                        _(f"Upgrading {obj_type} objects..."))
        ...
        for obj_type in OBJECT_TYPES:
            ...
            if blob_count != json_count:
                ...
                results['errors'].append(
                    f"{obj_type}: blob_count ({blob_count}) != "
                    f"json_count ({json_count})"
                )
