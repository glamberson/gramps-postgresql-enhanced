...
from debug_utils import (
    ...
    DebugContext, format_sql_query, timed_method
)
...
def test_query_profiler():
    ...
    queries = [
        ...
        ("INSERT INTO person (handle, json_data) VALUES (%s, %s)", ['I0002', '{}'], 0.020, 1),
        ("UPDATE person SET json_data = %s WHERE handle = %s", ['{}', 'I0001'], 0.015, 1),
        ("SELECT * FROM family WHERE handle IN %s", [['F0001', 'F0002']], 0.060, 2),  # Slow query
...
def test_transaction_tracker():
    ...
    print(f"Transaction {txn1_id} committed in {txn1['duration']:.3f}s with {len(txn1['operations'])} operations")
...
def test_debug_context():
    ...
    with debug_ctx.operation("import_gedcom", file="test.ged", records=1000) as op_id:
        ...
        # Log some queries
        debug_ctx.log_query("SELECT COUNT(*) FROM person", duration=0.005, rows_affected=1)
        debug_ctx.log_query("INSERT INTO person VALUES %s", params=['data'], duration=0.025, rows_affected=1)
        
        # Nested operation
        with debug_ctx.operation("process_families", count=50):
            debug_ctx.log_query("SELECT * FROM family WHERE handle = %s", 
                              params=['F0001'], duration=0.065, rows_affected=1)  # Slow query
            time.sleep(0.1)
...
def test_sql_formatter():
    ...
    test_cases = [
        ...
        ("INSERT INTO family (handle, json_data) VALUES (%s, %s)", ['F0001', '{"father": "I0001"}']),
        ("UPDATE person SET surname = %s, given_name = %s WHERE handle = %s", ['Smith', 'John', 'I0001']),
